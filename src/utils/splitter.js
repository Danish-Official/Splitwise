...
function downheapify(heap, i) {  // O(n)
    var largest = i;
    var lc = left(i);
    var rc = right(i);
...
// transactions = [{person1: 'ramesh', person2: 'naresh', amount: 100}]
export function splitter(transactions) {
    ...
    for (var i = 0; i < transactions.length; i++) {
        ...
        if (e.person1 in net_balance) {
            // checks whther e.person1 is present as key in hashmap or not
	...
	for(const p in net_balance) {
		if(net_balance[p] > 0) {
			...
			push_heap(negative, -1*net_balance[p], p);  // -1* because we need to pass absolute value for heap formation
		}
	}
